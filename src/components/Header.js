...
const Header = () => {
  ...
  const handleSearch = async (event) => {
    ...
    const response = await axios.get(
      `https://vinted2021.herokuapp.com/offers`,
      {
        title: title,
        priceMin: priceMin,
        priceMax: priceMax,
    ...
    console.log(response.data.offers[0].product_name.indexOf("switch"));
  };
    ...
    <div className="header">
      ...
      <nav>
        ...
        <input
          ...
          onChange={handleOrder}
        />
        <label for="descending"> D </label>
      </nav>
      <div className="buttons"></div>
      <button onClick={() => navigate("/signup")}>s'inscrire</button>
      <button onClick={() => navigate("/login")}>se connecter</button>
      <button onClick={handleConnect}>vends tes articles</button>
    </div>
  );
};
